...
class MovieController {
      ...
      try {
        ...
        if (!success) {
          ...
          throw new CreateFailedException(
            `Movie role(id: ${role.role_id}, role_type: ${role.role_type}) failed to be created`
          );
        }
      } catch (ex) {
        await DBService.rollback();
    ...
    for (const genre_id of genres) {
      ...
      try {
        ...
        if (!success) {
          await DBService.rollback();
          throw new CreateFailedException(
            `Movie genre(id: ${genre_id}) failed to be created`
          );
        }
      } catch (ex) {
        await DBService.rollback();
  ...
  update = async (body, id) => {
    ...
    if (!affectedRows) throw new NotFoundException("Movie not found");
      ...
      throw new UpdateFailedException("Movie update failed");
